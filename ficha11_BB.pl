...
-service(_, Name) :- 
    not(service(_, Name)),not(exception(service(_, Name))).
...
%Invariantes
+doing(A,P,U,_) :: (D\=feriado,findall((A,P,U,D),(doing(A,P,U,D)),S), length(S,N),N==1).

+service(S,_) :: (findall(S,(service(S,zulmira),not(null(S))),L),length(L,N),N==0).

-service(_,Nome) :: (findall(Nome,doing(_,Nome,_,_),S),length(S,N),N==0).
