...
exception(game(8,hh,V)) :- 
    about(1000,Sup,Low),
    V>=Low,
    V=<Sup.
exception(game(9,ii,Value)) :- 
    estimation(3000,Sup,Low),
    Value >= Low,
    Value =< Sup.
...
estimation(V,Sup,Low) :- 
    Sup is V*1.10,
    Low is V*0.90.

%Invariantes
+game(_,_,_) :: (
    findall(Help,(game(5,ee,Help),not(null(Help))),S),
    length(S,N),
    N == 0
                ).

+game(_,R,_) :: (
    findall(R,game(_,R,_),S),
    length(S,N),
    N=<3
               ).

+game(_,Ref,_) :: (
    findall((G1,G2),game(G1,Ref,_),
    game(G2,Ref,_),S),
    length(S,N),
    G2 is G1+1,
    N==0
